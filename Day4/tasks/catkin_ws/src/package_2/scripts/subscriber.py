...
def callback(num):
    rospy.loginfo("The counter: %s", num.data)

def listener():
    #creating the node:

    rospy.init_node("sub_counter_node", anonymous=True) # to  the node takes a uniqe name in the machine so even if some nodes have the same name, the program doesn't work well
    ...
    rospy.Subscriber("/countTopic", Int32, callback) #callback is the function that will be listened
